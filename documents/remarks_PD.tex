\documentclass[a4paper,final,oneside]{amsart}
...
\begin{document}
\begin{abstract}
Adiprasito, Huh, and Katz show the K\"ahler package for matroids using matroidal flips, which are purely combinatorial notions.
Following their work, Amini and Piquerez give a new proof of the K\"ahler package, more generally for quasi-projective unimodular Bergman fans using Keel's lemma and the weak factorization theorem for smooth toric varieties.
Later, in the matroid case, Hsiao shows that the weak factorization step can be replaced by an explicit sequence of toric blow-ups (i.e., star subdivisions of unimodular fans).
This is an expository paper reviewing a simple proof of Poincar\'e duality for matroids using this sequence of star subdivisions and Keel's lemma from algebraic geometry.
\end{abstract}
\maketitle
...
Adiprasito, Huh, and Katz show Poincar\'e duality of \(M\), which is part of the K\"ahler package, in the following sense.
...
Amini and Piquerez \cite[Section 3]{AP_20} develop a new approach to the K\"ahler package, more generally for quasi-projective unimodular Bergman fans, using tropical modifications, Keel's lemma, and the weak factorization theorem for smooth toric varieties.
See also \cite[Section 1.9]{AP_23} for a later account of this argument.
In the matroid case, the weak factorization step can be replaced by an explicit sequence of toric blow-ups.
Motivated by \cite{BHMPBW_22}, Hsiao, Karu, and Yang \cite{HKY_26} calculate mixed volumes of matroids using the deletion of matroids.
...
Using the factorization \eqref{eq_1}, Hsiao \cite{Hsiao_25} gives a simple combinatorial proof that \(A^{\bullet}(M)\otimes_{\symbf{Z}}\symbf{R}\) satisfies the hard Lefschetz property, the Hodge-Riemann relations, and therefore Poincar\'e duality.
...
With an aproopriately ordered basis chosen from each summand, the representation matrix of the bilinear pairing \(A^q\big(X_{\Sigma^*(\sigma)}\big)\times A^{r-q}\big(X_{\Sigma^*(\sigma)}\big)\to \symbf{Z}\) is a block lower triangular matrix 
\begin{equation}
\begin{bmatrix}
P_0 & O & \cdots &\cdots &O \\
...
The author would also like to thank Ryota Mikami for clarifying the historical background of the K\"ahler package for matroids.
The author is grateful to Kosuke Mizuno and Kentaro Tanaka for informative comments.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{bibdiv}
\begin{biblist}
\bib{AHK_18}{article}{
...
\bib{AP_20}{misc}{
   ...
   date={2020},
   note={arXiv preprint, \href{https://arxiv.org/abs/2007.07826v1}{arXiv:2007.07826v1}},
}
\bib{AP_23}{misc}{
   author={Amini, Omid},
   author={Piquerez, Matthieu},
   ...
   date={2023},
   note={arXiv preprint, \href{https://arxiv.org/abs/2310.15367v1}{arXiv:2310.15367v1}},
}
